...
def path3D(waypoints, T):
        ...
        for i in range((len(waypoints)-1)):
            ...
            while(elapsed < T):
                ...
                u2[0], u2[1], u2[2] = att_control3D(roll, roll_vel, pitch,pitch_vel, 0, yaw_vel, feedback_attitude)
                ...
                des_eq = [des_x_pos, des_y_pos, des_z_pos]

                elapsed = elapsed + dt  # check time to see if meets desired hovering time
                time_complete = (elapsed / T) * 100
                print("TIME Percent Complete")
                print(time_complete)
                poly.append(poly_eq)
                des.append(des_eq)
                print("desire position")
                print(des_x_pos, des_y_pos, des_z_pos)
                print("actual position")
                print(actual_x,actual_y,actual_z)
                print("actual vel")
                print(actual_xvel, actual_yvel, actual_zvel)
